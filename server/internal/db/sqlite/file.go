...
func (self *SQLiteManager) ListFiles(folderID uint64) ([]db.FileModel, error) {
	slog.Debug("Listing files", "folder_id", folderID, "db_type", "sqlite")
...
func (self *SQLiteManager) CreateFile(name string, folderID uint64, path string, storagePath string, location string, metadata []byte) (uint64, uint64, error) {
	now := uint64(time.Now().Unix())
	slog.Info("Creating file", "name", name, "folder_id", folderID, "path", path, "db_type", "sqlite")
...
func (self *SQLiteManager) GetFile(fileID uint64) (*db.FileModel, error) {
	slog.Debug("Getting file", "file_id", fileID, "db_type", "sqlite")
	var file db.FileModel
	var vid sql.NullInt64
...
func (self *SQLiteManager) GetFileByName(folderID uint64, name string) (*db.FileModel, error) {
	slog.Debug("Getting file by name", "folder_id", folderID, "name", name, "db_type", "sqlite")
	var file db.FileModel
	var vid sql.NullInt64
	err := self.db.QueryRow(`
		SELECT file_id, name, folder_id, path, storage_path, location, version_id, metadata, created_at, updated_at
...
func (self *SQLiteManager) CreateVersion(fileID uint64, version string, hash []byte, userID uint64, metadata []byte, comment string) (uint64, error) {
	now := uint64(time.Now().Unix())
	slog.Info("Creating file version", "file_id", fileID, "version", version, "user_id", userID, "db_type", "sqlite")
	result, err := self.db.Exec(`
...
func (self *SQLiteManager) UpdateFileVersion(fileID uint64, versionID uint64, storagePath string, location string) error {
	now := uint64(time.Now().Unix())
	slog.Info("Updating file with new version", "file_id", fileID, "version_id", versionID, "db_type", "sqlite")
	_, err := self.db.Exec(`
...
func (self *SQLiteManager) ListVersions(fileID uint64) ([]db.VersionModel, error) {
	slog.Debug("Listing file versions", "file_id", fileID, "db_type", "sqlite")
	rows, err := self.db.Query(`
...
func (self *SQLiteManager) DeleteOldVersions(fileID uint64, keepLastN uint32) (int64, error) {
	if keepLastN == 0 {
		return 0, nil
	}
	slog.Info("Deleting old file versions", "file_id", fileID, "keep_last_n", keepLastN, "db_type", "sqlite")
...
func (self *SQLiteManager) SoftDeleteFile(fileID uint64) error {
	now := uint64(time.Now().Unix())
	slog.Info("Soft deleting file", "file_id", fileID, "db_type", "sqlite")
...
func (self *SQLiteManager) RenameFile(fileID uint64, newName string) error {
	now := uint64(time.Now().Unix())
	slog.Info("Renaming file", "file_id", fileID, "new_name", newName, "db_type", "sqlite")
...
func (self *SQLiteManager) MoveFile(fileID uint64, newFolderID uint64, newPath string) error {
	now := uint64(time.Now().Unix())
	slog.Info("Moving file", "file_id", fileID, "new_folder_id", newFolderID, "new_path", newPath, "db_type", "sqlite")
...
func (self *SQLiteManager) UpdateFileMetadata(fileID uint64, metadata []byte) error {
	now := uint64(time.Now().Unix())
	slog.Info("Updating file metadata", "file_id", fileID, "db_type", "sqlite")
