...
func (self *SQLiteManager) EnsureRootFolder() error {
	slog.Debug("Ensuring root folder exists", "db_type", "sqlite")
	var count int
...
func (self *SQLiteManager) GetFolderPath(folderID uint64) (string, error) {
	slog.Debug("Getting folder path", "folder_id", folderID, "db_type", "sqlite")
	var path string
...
func (self *SQLiteManager) CreateFolder(name string, parentID uint64, path string, inheritance bool, retention uint32, metadata []byte) (uint64, uint64, error) {
	createdAt := uint64(time.Now().Unix())
	
	slog.Info("Creating folder", "name", name, "parent_id", parentID, "path", path, "db_type", "sqlite")
...
func (self *SQLiteManager) GetFolder(folderID uint64) (*db.FolderModel, error) {
	slog.Debug("Getting folder", "folder_id", folderID, "db_type", "sqlite")
...
func (self *SQLiteManager) SetRetentionPolicy(folderID uint64, lastN uint32) error {
	slog.Info("Setting retention policy", "folder_id", folderID, "last_n", lastN, "db_type", "sqlite")
	_, err := self.db.Exec(`
...
func (self *SQLiteManager) GetVersionRetention(folderID uint64) (uint32, error) {
	slog.Debug("Getting version retention", "folder_id", folderID, "db_type", "sqlite")
...
func (self *SQLiteManager) ListFolders(parentID uint64) ([]db.FolderModel, error) {
	slog.Debug("Listing folders", "parent_id", parentID, "db_type", "sqlite")
...
func (self *SQLiteManager) MoveFolder(folderID uint64, newParentID uint64, newPath string) error {
	slog.Info("Moving folder", "folder_id", folderID, "new_parent_id", newParentID, "new_path", newPath, "db_type", "sqlite")
...
func (self *SQLiteManager) SoftDeleteFolder(folderID uint64) error {
	slog.Info("Soft deleting folder", "folder_id", folderID, "db_type", "sqlite")
